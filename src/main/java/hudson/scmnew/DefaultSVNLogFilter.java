...
public class DefaultSVNLogFilter implements SVNLogFilter {
    ...
    /* (non-Javadoc)
	 * @see hudson.scmnew.SVNLogFilter#hasExclusionRule()
	 */
    ...
    /* (non-Javadoc)
	 * @see hudson.scmnew.SVNLogFilter#isIncluded(org.tmatesoft.svn.core.SVNLogEntry)
	 */
    public boolean isIncluded(SVNLogEntry logEntry) {
        if (excludedRevprop != null) {
            ...
            if (revprops != null && revprops.containsName(excludedRevprop)) {
                if (getLog() != null) {
                    getLog().println(Messages.SubversionSCM_pollChanges_ignoredRevision(
        ...
        if (includedPaths.size() == excludedPaths.size()) {
            if (getLog() != null) {
            	getLog().println(Messages.SubversionSCM_pollChanges_ignoredRevision(
                    logEntry.getRevision(),
                    Messages.SubversionSCM_pollChanges_ignoredRevision_path(String.join(", ", excludedPaths))));
            }
            return false;
        }
